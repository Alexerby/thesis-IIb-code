...
SOEP_STATES = {
    ...
    12: "Saarland", 13: "Sachsen", 14: "Sachsen-Anhalt", 15: "Schleswig-Holstein", 16: "Thüringen"
}
...
SHORT_NAMES = {
    ...
    "Niedersachsen": "NI", "Nordrhein-Westfalen": "NRW", "Rheinland-Pfalz": "RP",
...
def get_period_stats(df, years, treatment_var, state_var):
    ...
    stats = capable_df.groupby(state_var)[treatment_var].apply(
...
def plot_on_ax(ax, merged, treatment_var, title, vmin=40, vmax=90):
    ...
    for idx, row in merged.iterrows():
        ...
        text_color = 'white' if val > 65 else 'black' # Adjusted threshold for visual clarity
        ax.text(x + dx, y + dy, short_label, fontsize=7, ha='center', va='center', color=text_color, alpha=0.9)
...
def main():
    ...
    vmin, vmax = 40, 90
    plot_on_ax(ax1, pre_merged, treatment_var, f"Pre-COVID Average\n({min(pre_years)}–{max(pre_years)})", vmin, vmax)
    plot_on_ax(ax2, post_merged, treatment_var, f"Post-COVID Average\n({min(post_years)}–{max(post_years)})", vmin, vmax)
